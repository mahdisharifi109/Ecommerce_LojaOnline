        ...
        <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-xl animate-in fade-in duration-200">
          <div className="container mx-auto px-4 py-8 h-full flex flex-col">
            ...
            <div className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 overflow-y-auto pb-20">
              ...
              {/* Right Column: Results or Suggestions */}
              <div className="md:col-span-7 border-l border-border/30 md:pl-12">
                <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-6">
                  {searchQuery.length > 1 ? `Resultados para "${searchQuery}"` : "Sugestões para si"}
                </h3>
                ...
                {searchQuery.length > 1 && filteredProducts.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                    {(searchQuery.length > 1 ? filteredProducts : products.slice(0, 3)).map((product) => (
                      <div 
                        key={product.id} 
                      ...
                      >
                        ...
                        <h4 className="font-heading text-lg leading-tight group-hover:text-primary transition-colors line-clamp-1">
                          {product.name}
                        </h4>
